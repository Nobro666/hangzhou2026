...
class KinectCamera(Camera):
    ...
    def get_frame(self):
        ...
        return (True, color_img) if color_img is not None else (False, None)
        
    def get_depth(self):
        if not self.is_open:
            self.open_camera()
        capture = self.device.update()
        depth_img = capture.get_transformed_depth_image()
        return (True, depth_img) if depth_img is not None else (False, None)
