...
# 
    ...
    def catch_door(self):
        ...
        forward_position = [0.6232288384437561, -0.11755121499300003, 0.6069719791412354, 81.040, 83.972, 11.606]
        back_position = [0.5332288384437561, -0.11755121499300003, 0.6069719791412354, 81.040, 83.972, 11.606]
        back_position2 = [0.3332288384437561, -0.11755121499300003, 0.6069719791412354, 81.040, 83.972, 11.606]

        self.arm_run(pose_target=wait_position)
        # 移动到靠近位置
        self.arm_run(pose_target=forward_position)
        self.arm_run(finger_target=[20,20,20])
        #向前贴合
        self.arm_run(pose_target=[0.6332288384437561, -0.11755121499300003, 0.6069719791412354, 81.040, 83.972, 11.606])
        self.finger_run(finger_target=[85, 85, 85])
        self.finger_run(finger_target=[95, 95, 95])
        # 拉开门
        self.arm_run(pose_target=back_position)
        self.arm_run(pose_target=back_position2)
        # 松手
        self.finger_run(finger_target=[5, 5, 5])
        # 回正
        self.arm_run(pose_target=home_position)
    ...
    def put_bag(self):
        ...
        self.arm_run(pose_target=wait_position)
        self.arm_run(pose_target=ground_position)
        self.finger_run(finger_target=[5, 5, 5])
        self.arm_run(pose_target=home_position)
        rospy.sleep(2)
